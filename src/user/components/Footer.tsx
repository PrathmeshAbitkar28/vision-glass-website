...
const Footer = () => {
  ...
  // ── Same query key as Contact & FloatingButtons → shared cache, one fetch ──
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) { entry.target.classList.add("ft-visible"); observer.unobserve(entry.target); }
        });
      },
  ...
  return (
    <footer ref={footerRef} style={{ background: "rgb(2,6,23)", color: "#ffffff" }}>
      ...
      <div className="overflow-hidden" style={{ background: "#0ea5e9" }}>
        ...
        >
          {[...marqueeWords, ...marqueeWords].map((word, i) => (
            <span key={i} className="inline-flex items-center gap-2.5 px-5 text-[11px] font-bold uppercase tracking-[0.18em]" style={{ color: "rgb(7,18,36)" }}>
              {word}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-12 gap-6 lg:gap-6">
          ...
          <div className="ft-col col-span-2 md:col-span-1 lg:col-span-3">
            <div className="ft-logo flex items-center gap-2.5 mb-4">
              <img src={logo} alt="Vision Glass Creation" className="h-9 w-auto object-contain" style={{ filter: "brightness(0) invert(1)" }} />
              <div>
                <p className="font-extrabold text-white tracking-[-0.01em] leading-tight text-sm">Vision Glass</p>
                <p className="text-[9px] font-semibold tracking-[0.25em] uppercase" style={{ color: "#ffffff" }}>Creation</p>
              </div>
            </div>
            ...
            <p className="text-xs mb-5" style={{ color: "rgb(209,213,219)" }}>Proprietor: Pratap Bhagwanrao Kathare</p>
          ...
          <div className="ft-col hidden md:flex lg:col-span-2 flex-col items-start">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] mb-2.5" style={{ color: "#ffffff" }}>Scan to Visit</p>
            <div className="inline-flex flex-col items-center rounded-xl p-3 gap-2" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)" }}>
              <img
                src="/scanner_img/scanner.jpg" alt="Scan to visit on Google Maps"
                className="w-[100px] h-[100px] rounded-lg block" style={{ objectFit: "cover" }}
                onError={(e) => { (e.currentTarget as HTMLImageElement).src = "https://api.qrserver.com/v1/create-qr-code/?size=100x100&color=e0f2fe&bgcolor=020617&data=https://maps.google.com/?q=Plot+595+Ganganagar+Nigdi+Pimpri-Chinchwad"; }}
              />
              <p className="text-[10px] leading-tight text-center font-light" style={{ color: "rgb(209,213,219)" }}>Opens Google Maps</p>
            </div>
          </div>

          {/* Navigate */}
          <div className="ft-col lg:col-span-2">
            <h4 className="text-[11px] font-semibold uppercase tracking-[0.18em] mb-4" style={{ color: "#ffffff" }}>Navigate</h4>
          ...
          {/* Services */}
          <div className="ft-col lg:col-span-2">
            <h4 className="text-[11px] font-semibold uppercase tracking-[0.18em] mb-4" style={{ color: "#ffffff" }}>Our Services</h4>
            <div className="flex flex-col gap-2">
              {servicesList.map((s) => (
                <Link key={s} to="/services" className="text-sm font-light transition-colors duration-200"
                  style={{ color: "rgb(209,213,219)" }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = "#38bdf8"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = "rgb(209,213,219)"; }}
                >{s}</Link>
          ...
          {/* Contact */}
          <div className="ft-col col-span-2 md:col-span-1 lg:col-span-3">
            <h4 className="text-[11px] font-semibold uppercase tracking-[0.18em] mb-4" style={{ color: "#ffffff" }}>Contact</h4>
            <div className="flex flex-col gap-3">
              {contact?.phone && (
                <a href={`tel:${contact.phone.replace(/\s/g, "")}`} className="flex items-center gap-2.5 text-sm transition-colors duration-200" style={{ color: "rgb(209,213,219)" }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = "#38bdf8"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = "rgb(209,213,219)"; }}
                >
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)" }}>
              ...
              {contact?.officePhone && (
                <a href={`tel:${contact.officePhone.replace(/\s/g, "")}`} className="flex items-center gap-2.5 text-sm transition-colors duration-200" style={{ color: "rgb(209,213,219)" }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = "#38bdf8"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = "rgb(209,213,219)"; }}
                >
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)" }}>
              ...
              {contact?.email && (
                <a href={`mailto:${contact.email}`} className="flex items-center gap-2.5 text-sm transition-colors duration-200 break-all" style={{ color: "rgb(209,213,219)" }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = "#38bdf8"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = "rgb(209,213,219)"; }}
                >
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)" }}>
              ...
              {contact?.address && (
                <div className="flex items-start gap-2.5 text-sm" style={{ color: "rgb(209,213,219)" }}>
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)" }}>
          ...
          <div className="ft-col col-span-2 flex flex-col items-start md:hidden">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] mb-2" style={{ color: "#ffffff" }}>Scan to Visit</p>
            <div className="inline-flex flex-col items-center rounded-lg p-2 gap-1.5" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)" }}>
              <img
                src="/scanner_img/scanner.jpg" alt="Scan to visit on Google Maps"
                className="w-[72px] h-[72px] rounded-md block" style={{ objectFit: "cover" }}
                onError={(e) => { (e.currentTarget as HTMLImageElement).src = "https://api.qrserver.com/v1/create-qr-code/?size=72x72&color=e0f2fe&bgcolor=020617&data=https://maps.google.com/?q=Plot+595+Ganganagar+Nigdi+Pimpri-Chinchwad"; }}
              />
              <p className="text-[9px] leading-tight text-center font-light" style={{ color: "rgb(209,213,219)" }}>Opens Google Maps</p>
            </div>
          </div>
      ...
      {/* Divider */}
      <div ref={dividerRef} className="ft-divider mx-4 md:mx-8"
        style={{ height: "1px", background: "linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.2) 20%, rgba(255,255,255,0.2) 80%, transparent 100%)" }}
      />

      {/* Bottom bar */}
      <div ref={bottomRef} className="ft-bottom container mx-auto px-4 py-5 flex flex-col md:flex-row items-center justify-between gap-2">
        <p className="text-xs font-light" style={{ color: "rgba(209,213,219,0.8)" }}>Pune's Trusted Glass & Window Solutions Since 2009</p>
        <p className="text-xs font-light" style={{ color: "rgba(209,213,219,0.8)" }}>© {new Date().getFullYear()} Vision Glass Creation. All rights reserved.</p>
      </div>
